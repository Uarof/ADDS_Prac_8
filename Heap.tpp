...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            heapIndex current = this->tree.size() - 1;

            // Bubble up to restore heap property
            while (current > 1 && this->tree.at(current) < this->tree.at(getParentPosition(current))) {
            std::cout << "Swapping " << this->tree.at(current) << " with parent " << this->tree.at(getParentPosition(current)) << std::endl;
            std::swap(this->tree.at(current), this->tree.at(getParentPosition(current)));
        ...
        void remove(T value) {
            ...
            std::cout << "Removing element: " << this->tree.at(index) << std::endl;
            this->tree.at(index) = this->tree.back();
            this->tree.pop_back();

            // Try restoring heap by bubbling up or down
            if (index > 1 && this->tree.at(index) < this->tree.at(getParentPosition(index))) {
        ...
        T getMin() {
            ...
            return this->tree.at(1); // root is at index 1
        }
};
